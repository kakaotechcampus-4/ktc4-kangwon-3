...
| **403** | `FORBIDDEN` | 역할/정책 기반 인가 실패 | `@PreAuthorize` 등으로 막힌 관리자 전용 API에 일반 사용자가 접근함 |
...
> **타인 소유 리소스 접근도 404로 응답합니다.** 리소스가 존재하지만 요청자 소유가 아닌 경우(예: 타인이 등록한 엔티티 조회/수정/삭제 시도)에도 403이 아니라 404를 사용합니다. 403을 쓰면 "해당 ID가 실제로 존재한다"는 사실이 노출되어 ID 탐색·무차별 대입 공격에 악용될 수 있기 때문입니다(IDOR 방지). 403은 위 표처럼 리소스 존재 여부와 무관한 역할/정책 기반 인가 실패에만 사용합니다.
